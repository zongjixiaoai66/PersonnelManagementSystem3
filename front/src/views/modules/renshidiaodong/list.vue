        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '人事调动类型' : ''">
                        <el-select v-model="searchForm.renshidiaodongTypes" placeholder="请选择人事调动类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in renshidiaodongTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('renshidiaodong','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/renshiguanli/upload/renshidiaodongMuBan.xls"
                        >批量导入人事调动数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('renshidiaodong','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="renshidiaodongUploadSuccess"
                                :on-error="renshidiaodongUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('renshidiaodong','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入人事调动数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('renshidiaodong','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "renshidiaodong.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="renshidiaodongUuidNumber"
                                   header-align="center"
                                   label="人事调动编号">
                        <template slot-scope="scope">
                            {{scope.row.renshidiaodongUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="renshidiaodongTypes"
                                      header-align="center"
                                      label="人事调动类型">
                        <template slot-scope="scope">
                            {{scope.row.renshidiaodongValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('renshidiaodong','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('renshidiaodong','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('renshidiaodong','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              renshidiaodongTypesSelectSearch : [],
              bumenTypesSelectSearch : [],
              zhiweiTypesSelectSearch : [],
            form:{
                id : null,
                yuangongId : null,
                renshidiaodongUuidNumber : null,
                renshidiaodongTypes : null,
                diaodongTime : null,
                yuanBumen : null,
                yuanZhiwei : null,
                bumenTypes : null,
                zhiweiTypes : null,
                renshidiaodongContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '人事调动编号': "renshidiaodongUuidNumber",
                     '人事调动类型': "renshidiaodongValue",
                     '调动时间': "diaodongTime",
                     '原部门': "yuanBumen",
                     '原职位': "yuanZhiwei",
                     '部门': "bumenValue",
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'renshidiaodong',//当前表表名,
                        sumColum : 'renshidiaodong_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'renshidiaodong_name',//分组字符串字段
                        // types : 'renshidiaodong_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "renshidiaodong_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.renshidiaodongTypes!= '' && this.searchForm.renshidiaodongTypes!= undefined) {
                    params['renshidiaodongTypes'] = this.searchForm.renshidiaodongTypes
                }
                ...
                params['renshidiaodongDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "renshidiaodong/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=renshidiaodong_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.renshidiaodongTypesSelectSearch = data.data.list;
                    }
                });
                //填充下拉框选项
                this.$http({
            ...
            // 导入功能上传文件成功后调用导入方法
            renshidiaodongUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "renshidiaodong/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            renshidiaodongUploadError(data){
